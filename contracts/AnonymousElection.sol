...
contract AnonymousElection {
    ...
    constructor(
        ...
        require(
            ...
            "candidate list and voter list both need to have non-zero length"
        );
        name = _name;
        round = 1;
        owner = _owner;
    ...
    // returns bytes2048 for the Zero-Knowledge proof in submitPK
    ...
    function submitPK(
        ...
        if (submittedPKs == voters.length) {
            // if everyone has submitted their pk
    ...
    function vote(bytes memory _encVote) public {
        ...
        if (submittedVotes == voters.length) {
            // if everyone has submitted their vote
